...
const analyze = sg.wrap(async (args: { address: string }) => {
  ...
  },
  {
    ...
    integration:
      "SettleGrid acts as an AP2 credentials provider. When an agent presents AP2 credentials, SettleGrid verifies the SettleGrid-issued AP2 credential (the issuer must be settlegrid.ai — there is no call to Google's infrastructure), checks the embedded budget cap, and executes your tool. Developers do not need to implement any AP2-specific logic — the SettleGrid protocol adapter handles credential verification and budget checking. AP2 settlement is dark-gated and in development.",
...
const analyze = sg.wrap(async (args: { documentUrl: string }) => {
  ...
  },
  {
    ...
    integration:
      "SettleGrid supports Visa TAP tokens as an identity type in its KYA (Know Your Agent) system. When an agent presents a TAP token, SettleGrid verifies it against Visa's VTS endpoint (sandbox by default), maps the token to an internal consumer identity, checks credit balance, and — when configured for production — authorizes (but does not capture) a Visa charge, then meters the invocation. Developers receive payouts through Stripe Connect as usual.",
...
const searchFlights = sg.wrap(async (args: { origin: string; dest: string }) => {
  ...
  },
  {
    ...
    integration:
      "SettleGrid exposes your tool as a UCP-compatible service. When a UCP agent discovers your tool in the catalog, SettleGrid returns your pricing, terms, and availability. When the agent initiates a purchase, SettleGrid detects the UCP session and meters the invocation. UCP is a detection-only adapter today — live settlement is dark-gated and in development. Developers get UCP catalog visibility without building any UCP-specific infrastructure.",
...
const securityScan = sg.wrap(async (args: { repoUrl: string }) => {
  ...
  },
  {
    ...
    integration:
      "SettleGrid ships a Mastercard Verifiable Intent detection adapter. This rail is not yet live: presented Verifiable Intent tokens are detected, but validation returns not-yet-supported, so no payment is processed. When Mastercard Verifiable Intent reaches general availability, SettleGrid will validate the token with Mastercard's infrastructure, check that your tool's merchant category is allowed, verify the spending limit, and process the payment. Developers receive payouts through Stripe Connect as with all other protocols.",

...
export default function AcademyLandingError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('Academy landing page render error:', error)
  }, [error])
      ...
      <main className="flex-1 px-6 py-12 flex items-center justify-center">
        <div className="max-w-md w-full bg-[#161822] border border-[#2A2D3E] rounded-xl p-8 text-center">
          ...
          <h2 className="text-lg font-semibold text-gray-100 mb-2">
            Academy landing failed to render
          </h2>
          <p className="text-sm text-gray-400 mb-6">
            Something went wrong while loading the Academy landing
            page. Individual lesson pages are unaffected.
          </p>
          {error.digest && (

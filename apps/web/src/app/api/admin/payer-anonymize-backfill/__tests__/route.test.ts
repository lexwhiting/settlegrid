/**
 * V-N3-erasure — one-time payer-anonymize BACKFILL admin route (F3: a flag-gated
 * ADMIN ROUTE, never a script). Cron-secret auth gating + DARK-flag pin (flag OFF
 * ⇒ zero DB access through the route) + delegation to runPayerAnonymization (same
 * shared transform/predicate as the cron). Uses the REAL verifyCronAuth + env
 * getters and a mocked db, so the dark gate is proven end-to-end through the route.
 */
...
function req(auth?: string): NextRequest {
  ...
  return new NextRequest('http://localhost/api/admin/payer-anonymize-backfill', { method: 'POST', headers })
}
...
describe('payer-anonymize backfill admin route', () => {
  ...
  it('flag ON + empty table ⇒ 200 enabled:true, anonymized:0, completed:true, table scanned', async () => {

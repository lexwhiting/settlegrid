        ...
        .set({
          ...
          resultUrl: JSON.stringify({
            ...
            retainedUnscrubbed: [
              ...
              // (mirrors the SLICE-4 consumer-linkage retainedUnscrubbed→anonymized
              // gating above — a path conditionally bucketed by whether its scrub ran).
            ...
            ...(payerMinimizeEnabled
              ...
              : {}),
            // ③ deep-audit F1: the foreign-tool-invocation clause is GATED on
            // `consumerMatched`. Without a consumer twin, step 2 never ran and there are
            // no twin-keyed foreign-tool rows, so asserting their retention + a step-2
            // pseudonymization would be a FALSE PARTICULAR in the persisted record. Both
            // flag branches keep the literal clause in source (inside the conditional),
            // so the source-text honesty pins (compliance-honesty-regression.test.ts)
            // stay valid; the runtime omission for no-twin subjects is pinned in the
            // integration test ("NO-TWIN DISCLOSURE (F1)").

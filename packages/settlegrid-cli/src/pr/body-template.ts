import type { OpenPrInput } from './github.js'

/**
 * Shape consumed by renderPrBody — strictly the fields of OpenPrInput
 * that are safe to publish in a public pull-request body. Derived
 * via `Omit` (per P2.4 spec) so any future field added to OpenPrInput
 * automatically flows into renderPrBody's signature without a parallel
 * interface that might drift.
 *
 * Omitted: `token` (secret), `baseBranch` + `branchName` (git-layer
 * internals irrelevant to the reader of the PR description).
 */
export type RenderPrBodyInput = Omit<
  OpenPrInput,
  'token' | 'baseBranch' | 'branchName'
>

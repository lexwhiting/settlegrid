/**
 * settlegrid-switzerland-data — Swiss Open Data MCP Server
 *
 * Wraps opendata.swiss CKAN with SettleGrid billing.
...
const API_BASE = 'https://ckan.opendata.swiss/api/3'
const USER_AGENT = 'settlegrid-switzerland-data/1.0 (contact@settlegrid.ai)'
...
async function apiFetch<T>(path: string, options: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`opendata.swiss CKAN API ${res.status}: ${body.slice(0, 200)}`)
  }
...
const sg = settlegrid.init({
  toolSlug: 'switzerland-data',
  pricing: {
    defaultCostCents: 1,
    methods: {
      search_datasets: { costCents: 1, displayName: 'Search Swiss Open Data datasets' },
...
console.log('settlegrid-switzerland-data MCP server ready')

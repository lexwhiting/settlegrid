/**
 * settlegrid-germany-data — German Open Data MCP Server
 *
 * Wraps GovData.de CKAN with SettleGrid billing.
...
const API_BASE = 'https://ckan.govdata.de/api/3'
const USER_AGENT = 'settlegrid-germany-data/1.0 (contact@settlegrid.ai)'
...
async function apiFetch<T>(path: string, options: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`GovData.de CKAN API ${res.status}: ${body.slice(0, 200)}`)
  }
...
const sg = settlegrid.init({
  toolSlug: 'germany-data',
  pricing: {
    defaultCostCents: 1,
    methods: {
      search_datasets: { costCents: 1, displayName: 'Search German Open Data datasets' },
...
console.log('settlegrid-germany-data MCP server ready')

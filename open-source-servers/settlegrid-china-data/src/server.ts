/**
 * settlegrid-china-data — Chinese Economic Data MCP Server
 *
 * Wraps World Bank API (CHN subset) with SettleGrid billing.
 * No API key needed for the upstream service.
 *
 * Methods:
 *   get_indicators(indicator)              — Get indicator data (1¢)
 *   list_indicators(topic?)                — List available indicators (1¢)
 *   get_gdp_data(year?)   — Get GDP data for China (1¢)
 */
...
const COUNTRY = 'CHN'
const USER_AGENT = 'settlegrid-china-data/1.0 (contact@settlegrid.ai)'
...
const sg = settlegrid.init({
  ...
  pricing: {
    defaultCostCents: 1,
    methods: {
      get_indicators: { costCents: 1, displayName: 'Get indicator data for Chinese' },
      list_indicators: { costCents: 1, displayName: 'List available indicators' },
      get_gdp_data: { costCents: 1, displayName: 'Get GDP data for China' },
    },
  },
})
...
console.log('settlegrid-china-data MCP server ready')

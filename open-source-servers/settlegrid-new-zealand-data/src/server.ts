/**
 * settlegrid-new-zealand-data — New Zealand Open Data MCP Server
 *
 * Wraps data.govt.nz CKAN with SettleGrid billing.
...
const API_BASE = 'https://catalogue.data.govt.nz/api/3'
const USER_AGENT = 'settlegrid-new-zealand-data/1.0 (contact@settlegrid.ai)'
...
async function apiFetch<T>(path: string, options: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`data.govt.nz CKAN API ${res.status}: ${body.slice(0, 200)}`)
  }
...
const sg = settlegrid.init({
  toolSlug: 'new-zealand-data',
  pricing: {
    defaultCostCents: 1,
    methods: {
      search_datasets: { costCents: 1, displayName: 'Search New Zealand Open Data datasets' },
...
console.log('settlegrid-new-zealand-data MCP server ready')

/**
 * settlegrid-israel-data — Israeli Open Data MCP Server
 *
 * Wraps data.gov.il CKAN with SettleGrid billing.
...
const API_BASE = 'https://data.gov.il/api/3'
const USER_AGENT = 'settlegrid-israel-data/1.0 (contact@settlegrid.ai)'
...
async function apiFetch<T>(path: string, options: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`data.gov.il CKAN API ${res.status}: ${body.slice(0, 200)}`)
  }
...
const sg = settlegrid.init({
  toolSlug: 'israel-data',
  pricing: {
    defaultCostCents: 1,
    methods: {
      search_datasets: { costCents: 1, displayName: 'Search Israeli Open Data datasets' },
...
console.log('settlegrid-israel-data MCP server ready')

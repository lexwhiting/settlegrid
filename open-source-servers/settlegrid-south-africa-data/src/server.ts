/**
 * settlegrid-south-africa-data — South African Open Data MCP Server
 *
 * Wraps data.gov.za CKAN with SettleGrid billing.
...
const API_BASE = 'https://data.gov.za/api/3'
const USER_AGENT = 'settlegrid-south-africa-data/1.0 (contact@settlegrid.ai)'
...
async function apiFetch<T>(path: string, options: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`data.gov.za CKAN API ${res.status}: ${body.slice(0, 200)}`)
  }
...
const sg = settlegrid.init({
  toolSlug: 'south-africa-data',
  pricing: {
    defaultCostCents: 1,
    methods: {
      search_datasets: { costCents: 1, displayName: 'Search South African Open Data datasets' },
...
console.log('settlegrid-south-africa-data MCP server ready')

/**
 * settlegrid-polygonscan — PolygonScan MCP Server
 *
...
async function apiFetch<T>(path: string): Promise<T> {
  const res = await fetch(`${BASE}${path}`, {
    headers: { 'User-Agent': 'settlegrid-polygonscan/1.0' },
  })
  if (!res.ok) {
    const body = await res.text().catch(() => '')
    throw new Error(`PolygonScan API ${res.status}: ${body.slice(0, 200)}`)
  }
...
const sg = settlegrid.init({
  toolSlug: 'polygonscan',
  pricing: {
    defaultCostCents: 2,
    methods: {
      get_balance: { costCents: 2, displayName: 'MATIC Balance' },
...
console.log('settlegrid-polygonscan MCP server ready')

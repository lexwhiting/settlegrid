/**
 * settlegrid-peru-data — Peruvian Data MCP Server
 *
 * Wraps World Bank API (PER subset) with SettleGrid billing.
 * No API key needed for the upstream service.
 *
 * Methods:
 *   get_indicators(indicator)              — Get indicator data (1¢)
 *   list_indicators(topic?)                — List available indicators (1¢)
 *   get_population_data(year?)   — Get population data for Peru (1¢)
 */
...
const COUNTRY = 'PER'
const USER_AGENT = 'settlegrid-peru-data/1.0 (contact@settlegrid.ai)'
...
const sg = settlegrid.init({
  ...
  pricing: {
    defaultCostCents: 1,
    methods: {
      get_indicators: { costCents: 1, displayName: 'Get indicator data for Peruvian' },
      list_indicators: { costCents: 1, displayName: 'List available indicators' },
      get_population_data: { costCents: 1, displayName: 'Get population data for Peru' },
    },
  },
})
...
console.log('settlegrid-peru-data MCP server ready')

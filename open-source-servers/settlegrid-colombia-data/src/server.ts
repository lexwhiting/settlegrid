/**
 * settlegrid-colombia-data — Colombian Data MCP Server
 *
 * Wraps World Bank API (COL subset) with SettleGrid billing.
 * No API key needed for the upstream service.
 *
 * Methods:
 *   get_indicators(indicator)              — Get indicator data (1¢)
 *   list_indicators(topic?)                — List available indicators (1¢)
 *   get_gdp_data(year?)   — Get GDP data for Colombia (1¢)
 */
...
const COUNTRY = 'COL'
const USER_AGENT = 'settlegrid-colombia-data/1.0 (contact@settlegrid.ai)'
...
const sg = settlegrid.init({
  toolSlug: 'colombia-data',
  pricing: {
    defaultCostCents: 1,
    methods: {
      get_indicators: { costCents: 1, displayName: 'Get indicator data for Colombian' },
      list_indicators: { costCents: 1, displayName: 'List available indicators' },
      get_gdp_data: { costCents: 1, displayName: 'Get GDP data for Colombia' },
    },
  },
})
...
console.log('settlegrid-colombia-data MCP server ready')

/**
 * settlegrid-bscscan — BscScan MCP Server
 *
...
async function apiFetch<T>(path: string): Promise<T> {
  const res = await fetch(`${BASE}${path}`, {
    headers: { 'User-Agent': 'settlegrid-bscscan/1.0' },
  })
  if (!res.ok) {
    const body = await res.text().catch(() => '')
    throw new Error(`BscScan API ${res.status}: ${body.slice(0, 200)}`)
  }
...
const sg = settlegrid.init({
  ...
  pricing: {
    defaultCostCents: 2,
    methods: {
      get_balance: { costCents: 2, displayName: 'BNB Balance' },
...
console.log('settlegrid-bscscan MCP server ready')

/**
 * settlegrid-chile-data — Chilean Data MCP Server
 *
 * Wraps World Bank API (CHL subset) with SettleGrid billing.
 * No API key needed for the upstream service.
 *
 * Methods:
 *   get_indicators(indicator)              — Get indicator data (1¢)
 *   list_indicators(topic?)                — List available indicators (1¢)
 *   get_gdp_data(year?)   — Get GDP data for Chile (1¢)
 */
...
const COUNTRY = 'CHL'
const USER_AGENT = 'settlegrid-chile-data/1.0 (contact@settlegrid.ai)'
...
const sg = settlegrid.init({
  ...
  pricing: {
    defaultCostCents: 1,
    methods: {
      get_indicators: { costCents: 1, displayName: 'Get indicator data for Chilean' },
      list_indicators: { costCents: 1, displayName: 'List available indicators' },
      get_gdp_data: { costCents: 1, displayName: 'Get GDP data for Chile' },
    },
  },
})
...
console.log('settlegrid-chile-data MCP server ready')

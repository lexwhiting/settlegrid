/**
 * settlegrid-uk-gov-data — UK Government Datasets MCP Server
 *
 * Wraps UK Government CKAN with SettleGrid billing.
...
const API_BASE = 'https://ckan.publishing.service.gov.uk/api/3'
const USER_AGENT = 'settlegrid-uk-gov-data/1.0 (contact@settlegrid.ai)'
...
async function apiFetch<T>(path: string, options: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`UK Government CKAN API ${res.status}: ${body.slice(0, 200)}`)
  }
...
const sg = settlegrid.init({
  toolSlug: 'uk-gov-data',
  pricing: {
    defaultCostCents: 1,
    methods: {
      search_datasets: { costCents: 1, displayName: 'Search UK Government Datasets datasets' },
...
console.log('settlegrid-uk-gov-data MCP server ready')

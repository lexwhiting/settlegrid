/**
 * settlegrid-kenya-data — Kenyan Open Data MCP Server
 *
 * Wraps openAFRICA CKAN with SettleGrid billing.
...
const API_BASE = 'https://open.africa/api/3'
const USER_AGENT = 'settlegrid-kenya-data/1.0 (contact@settlegrid.ai)'
...
async function apiFetch<T>(path: string, options: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`openAFRICA CKAN API ${res.status}: ${body.slice(0, 200)}`)
  }
...
const sg = settlegrid.init({
  ...
  pricing: {
    defaultCostCents: 1,
    methods: {
      search_datasets: { costCents: 1, displayName: 'Search Kenyan Open Data datasets' },
...
console.log('settlegrid-kenya-data MCP server ready')

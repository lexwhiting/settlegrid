/**
 * settlegrid-thailand-data — Thai Statistics MCP Server
 *
 * Wraps World Bank API (THA subset) with SettleGrid billing.
 * No API key needed for the upstream service.
 *
 * Methods:
 *   get_indicators(indicator)              — Get indicator data (1¢)
 *   list_indicators(topic?)                — List available indicators (1¢)
 *   get_population_data(year?)   — Get population data for Thailand (1¢)
 */
...
const COUNTRY = 'THA'
const USER_AGENT = 'settlegrid-thailand-data/1.0 (contact@settlegrid.ai)'
...
const sg = settlegrid.init({
  toolSlug: 'thailand-data',
  pricing: {
    defaultCostCents: 1,
    methods: {
      get_indicators: { costCents: 1, displayName: 'Get indicator data for Thai' },
      list_indicators: { costCents: 1, displayName: 'List available indicators' },
      get_population_data: { costCents: 1, displayName: 'Get population data for Thailand' },
    },
  },
})
...
console.log('settlegrid-thailand-data MCP server ready')
